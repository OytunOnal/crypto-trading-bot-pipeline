"""RSI Mean-Reversion 5m cache builder (year-split, common interface). TOY example."""
import sys
...
class RsiMrCache(StrategyCache):
    CACHE_FILE = Path(__file__).resolve().parent / 'cache' / 'rsi_mr_5m_raw_cache.pkl'
    DESCRIPTION = 'RSI Mean-Reversion 5m (toy)'

    def compute_signals(self, df):
        long_mask, short_mask = compute_rsi_mr_signals(df['close'].values)
        return long_mask, short_mask


_instance = RsiMrCache()
